...
spec = do
  describe "runCPU" $ do
    ...
    describe "2nnn" $ do
      ...
      it "should increment the stack pointer" $ do
        sp resultState `shouldBe` sp initialState + 1
      it "should set the current pc to the top of the stack" $ do
        (U.!) (stack resultState) (sp resultState) `shouldBe` pc initialState
      it "should set the current pc to be NNN + 2" $ do
        (pc resultState) `shouldBe` 2748 + 2
    describe "3xkk" $ do
      describe "x == kk" $ do
        let newStack = U.replicate 12 255
        let (resultState, _) = runCPU "3AFF" (initialState {stack = newStack}, (U.replicate 10 1))
        it "should set the next pc to be current pc + 4" $ do
          (pc resultState) `shouldBe` (pc initialState) + 4
      describe "x != kk" $ do
        let newStack = U.replicate 12 0
        let (resultState, _) = runCPU "3AFF" (initialState {stack = newStack}, (U.replicate 10 1))
        it "should set the next pc to be current pc + 2" $ do
          (pc resultState) `shouldBe` (pc initialState) + 2
